...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_stmt = $conn->prepare("SELECT COUNT(*) FROM announcements WHERE title = ?");
    ...
    if ($count > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO announcements (title, content, image, start_date, end_date) VALUES (?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("sssss", $title, $content, $image, $start_date, $end_date);
            if ($stmt->execute()) {
